...
let fallbackDiffPromise:
  | Promise<typeof import('./view-model/run-markdown-diff').runMarkdownDiff>
  | undefined
...
export function useDiffWorkbench(initialOldMarkdown: string, initialNewMarkdown: string) {
  ...
  const pendingScrollTarget = ref<{ changeKey: string; index: number; side: 'old' | 'new' } | null>(
    null,
  )
  ...
  const projectionLines = computed<ProjectionLine[]>(() =>
    result.value
      ? buildProjectionLines(newMarkdown.value, result.value, flatChanges.value)
      : buildEmptyLines(newMarkdown.value),
  )
  const oldProjectionLines = computed<ProjectionLine[]>(() =>
    result.value
      ? buildOldProjectionLines(oldMarkdown.value, result.value, flatChanges.value)
      : buildEmptyLines(oldMarkdown.value),
  )
  ...
  const debugSnapshot = computed(() =>
    result.value ? buildDebugSnapshot(result.value) : undefined,
  )
  ...
  const isDiffStale = computed(
    () =>
      result.value !== null &&
      (oldMarkdown.value !== lastDiffedOld.value || newMarkdown.value !== lastDiffedNew.value),
  )
    ...
    const cards: StatCardModel[] = [
      {
        key: 'insert',
        label: '新增',
        value: result.value.stats.inserts,
        filter: 'insert',
        description: '仅新文档存在的内容。',
        onClick: () => scrollToFirstMatch('insert'),
      },
      {
        key: 'delete',
        label: '删除',
        value: result.value.stats.deletes,
        filter: 'delete',
        description: '仅旧文档存在的内容。',
        onClick: () => scrollToFirstMatch('delete'),
      },
      {
        key: 'replace',
        label: '替换',
        value: result.value.stats.replaces,
        filter: 'replace',
        description: '已配对但内容发生变化的区域。',
        onClick: () => scrollToFirstMatch('replace'),
      },
      {
        key: 'move',
        label: '移动',
        value: result.value.stats.moves,
        filter: 'move',
        description: '内容被识别为移动，而不是删后新增。',
        onClick: () => scrollToFirstMatch('move'),
      },
      {
        key: 'reorder',
        label: '重排',
        value: result.value.stats.reorders,
        filter: 'reorder',
        description: '兄弟节点顺序发生变化。',
        onClick: () => scrollToFirstMatch('reorder'),
      },
      {
        key: 'meta',
        label: '元数据',
        value: result.value.stats.metaUpdates,
        filter: 'meta',
        description: '结构、frontmatter 或代码围栏元数据变化。',
        onClick: () => scrollToFirstMatch('meta'),
      },
      {
        key: 'rename',
        label: '改名',
        value: result.value.stats.renames,
        filter: 'rename',
        description: '标题或引用标识符发生重命名。',
        onClick: () => scrollToFirstMatch('rename'),
      },
      {
        key: 'warning',
        label: '提示',
        ...
        filter: 'warning',
        description: '存在降级、预算或一致性提示。',
        onClick: () => scrollToFirstMatch('warning'),
      },
    ]
    if (q.degradedCount > 0)
      cards.push({
        key: 'degraded',
        label: '降级',
        value: q.degradedCount,
        filter: 'warning',
        description: '部分区域使用了简化对齐。',
        onClick: () => scrollToFirstMatch('warning'),
      })
    if (q.inlineDeferredCount > 0)
      cards.push({
        key: 'deferred',
        label: '延后',
        value: q.inlineDeferredCount,
        filter: 'warning',
        description: '片段级高亮因内容过长而退化。',
        onClick: () => scrollToFirstMatch('warning'),
      })
    return cards
  })
    ...
    try {
      ...
      const raw =
        typeof Worker !== 'undefined'
          ...
          : await (
              await loadRunMarkdownDiff()
            )(oldMarkdown.value, newMarkdown.value)
...
async function loadRunMarkdownDiff(): Promise<
  typeof import('./view-model/run-markdown-diff').runMarkdownDiff
> {
  fallbackDiffPromise ??= import('./view-model/run-markdown-diff').then(
    (module) => module.runMarkdownDiff,
  )
